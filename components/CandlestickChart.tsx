...
const CandlestickChart = ({
    ...
    /**
     * ✅ FETCH OHLC DATA (FREE API SAFE)
     */
    const fetchOHLCData = async (selectedPeriod: Period) => {
        try {
            const { days } = PERIOD_CONFIG[selectedPeriod];

            // ✅ FREE CoinGecko API request
    ...
    useEffect(() => {
        ...
        series.setData(convertOHLCData(converted));
    ...
    useEffect(() => {
        ...
        if (liveOhlcv) {
            ...
            if (last && last[0] === liveTimestamp) {
                merged = [...converted.slice(0, -1), liveOhlcv];
            } else {
                merged = [...converted, liveOhlcv];
            }
        } else {
            merged = converted;
        }
